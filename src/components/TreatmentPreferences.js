...
import { Info, ChevronDown, ChevronUp, Edit2, Check, CheckCircle } from 'lucide-react';
  ...
  useEffect(() => {
    const loadPreferences = async () => {
      try {
        ...
        // Do NOT set registrationComplete. Instead, rely on user?.registrationStep.
        // We still load preferences from the server response:
        if (data.profile?.treatmentPreferences?.bodyAreas) {
          const loadedPreferences = {};
          Object.entries(data.profile.treatmentPreferences.bodyAreas).forEach(([key, value]) => {
        ...
        // If user needs to complete step 1 before step 2, we still redirect:
        // This logic checks if data says user.registrationStep < 2, but ideally we should rely on `user`.
        // For now, we’ll keep this since user might not be loaded yet in AuthContext at this point.
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const formattedPreferences = {
        bodyAreas: Object.entries(selectedAreas).reduce((acc, [key, value]) => {
          ...
          <div className="flex justify-between items-center mb-8">
            <div className="text-center">
              ...
              {!isRegistrationComplete && (
                <p className="mt-2 text-slate-500">Step 3 of 3: Customize your massage experience</p>
              )}
            </div>
            {isRegistrationComplete && (
              <button
